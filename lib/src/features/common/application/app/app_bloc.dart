...
class AppBloc extends Bloc<AppEvent, AppState> {
  AppBloc(this._logger, this._settingsService, this._deviceInfoService) : super(const _AppLoadingState()) {
  ...
  AppState _loadedState(AppThemeType theme, AutoThemeModeType autoThemeMode, {bool isInitialized = true}) {
  ...
  Future<void> _mapInitToState(_Init event, Emitter<AppState> emit, {bool init = true}) async {
  ...
  Future<void> _mapThemeChangedToState(_ThemeChanged event, Emitter<AppState> emit) async {
    _logInfo();

    emit(_loadedState(event.newValue, _settingsService.autoThemeMode));
  }

  Future<void> _mapThemeModeChangedToState(_ThemeModeChanged event, Emitter<AppState> emit) async {
    _logInfo();
