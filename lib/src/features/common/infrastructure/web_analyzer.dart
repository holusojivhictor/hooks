...
class WebAnalyzer {
  ...
  static Future<InfoBase?> getInfo({
    ...
    final key = getKey(story);
    final url = story.url;

    var info = getInfoFromCache(key);
  ...
  static Future<InfoBase?> _getInfoByIsolate({
    ...
    if (res == null || isEmpty(res[2] as String?)) {
      final commentText = await compute(
        _fetchInfoFromStory,
  ...
  static Future<String?> _fetchInfoFromStory(List<int> meta) async {
    ...
    final storyId = meta.first;
    var kids = meta.sublist(1, meta.length);
    ...
    if (kids.isEmpty) {
      final story = await storiesService.fetchStory(id: storyId);
    ...
    final comment = await storiesService.fetchComment(id: kids.first);
  ...
  static String? _analyzeDescription(Document document, String html) {
    ...
    if (isEmpty(description)) {
      var body = html.replaceAll(_htmlReg, '');
  ...
  static String? _analyzeIcon(Document document, Uri uri) {
    ...
    String? icon = '';
    // get icon first
    var metaIcon = meta.firstWhereOrNull((Element e) {
