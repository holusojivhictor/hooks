...
class CollapseCache {
  ...
  final PublishSubject<Map<int, Set<int>>> _hiddenCommentsSubject = PublishSubject<Map<int, Set<int>>>();

  Stream<Map<int, Set<int>>> get hiddenComments => _hiddenCommentsSubject.stream;
  ...
  Set<int> collapse(int commentId) {
    ...
    Set<int> findHiddenComments(int commentId) {
      final directKids = <int>{...?_kids[commentId]};
      final temp = <int>{...directKids};
    ...
    final hiddenComments = findHiddenComments(commentId);
  ...
  bool isHidden(int commentId) {
    for (final val in _hidden.values) {
